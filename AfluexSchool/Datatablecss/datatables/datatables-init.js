$(document).ready(function () {
    ...
    $(document).ready(function () {
        var table = $('#example').DataTable({
            ...
            "order": [[2, 'asc']], "displayLength": 25,
            "drawCallback": function (settings) {
                var api = this.api();
                var rows = api.rows({ page: 'current' }).nodes();
                var last = null; api.column(2, { page: 'current' }).data().each(function (group, i) { if (last !== group) { $(rows).eq(i).before('<tr class="group"><td colspan="5">' + group + '</td></tr>'); last = group; } });
            }
...
$('#example24').DataTable({ dom: 'Bfrtip', buttons: [], "displayLength": 50 });
